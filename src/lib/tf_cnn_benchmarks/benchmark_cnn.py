...
# Params directly.
Params = namedtuple('Params', _DEFAULT_PARAMS.keys()
...
def make_params_from_flags():
    ...
    # _DEFAULT_PARAMS.
    flag_values = {name: getattr(FLAGS, name)
                   for name in _DEFAULT_PARAMS.keys()}
...
class BenchmarkCNN(object):
    ...
    def __init__(self, params):
        ...
        self.model = model_config.get_model_config(
            self.params.model, self.dataset)
        ...
        if self.params.gpu_indices:
            self.gpu_indices = [int(x)
                                for x in self.params.gpu_indices.split(',')]
        else:
    ...
    def _benchmark_cnn(self):
        ...
        with sv.managed_session(
            ...
            if self.single_session or (self.params.cross_replica_sync and
                ...
                # local steps, or else the workers running the extra step will
                # block.
                def done_fn(): return local_step == self.num_batches
            else:
            ...
            if is_chief:
                store_benchmarks(
                    {'total_images_per_sec': images_per_sec}, self.params)
            # Save the model checkpoint.
            if self.params.train_dir is not None and is_chief:
                checkpoint_path = os.path.join(
                    self.params.train_dir, 'model.ckpt')
        ...
        for d, device in enumerate(apply_gradient_devices):
            with tf.device(device):
                ...
                if ((not self.use_synthetic_gpu_images) and
                    ...
                    # Decay the learning rate exponentially based on the number
                    # of steps.
        ...
        with tf.device(self.cpu_device):
            if self.task_index == 0 and self.params.summary_verbosity >= 1:
                ...
                if self.params.summary_verbosity >= 2:
                    ...
                    indices_for_non_zero_grads = tf.where(
                        tf.not_equal(grads, 0))
    ...
    def add_forward_pass_and_gradients(
        ...
        with tf.device(self.devices[rel_device_num]):
            ...
            grads = tf.gradients(
                scaled_loss, params, aggregation_method=aggmeth)
            if self.loss_scale != 1.:
                ...
                # compatible with modifying both the gradients and the learning
                # rate.
            ...
            if self.params.staged_vars:
                ...
                grad_stage = data_flow_ops.StagingArea(
    ...
    def add_sync_queues_and_barrier(self, name_prefix,
        ...
        with tf.device(self.sync_queue_devices[
            ...
            # Drain tokens off queue for this worker, one for each other
            # worker.
            queue_ops.append(
...
def store_benchmarks(names_to_values, params):
    if params.result_storage:
        benchmark_storage.store_benchmark(
            names_to_values, params.result_storage)
