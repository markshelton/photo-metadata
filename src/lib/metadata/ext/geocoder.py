...
def prepare_search_strings():
    ...
    stop_words = make_candidate_string(stop_words_dict["stop_word"], lower=False)
...
def parse_address(location_text):
    ...
    re_main = re.compile(
        ...
        r'(?P<suburb_name>\b(?:{0})\b)'.format(suburb_names)
        , re.IGNORECASE
    )
...
def create_structured_params(street_number=None, street_name=None, street_type=None, 
    ...
    params_dict = {}
    if street_number:
        params_dict["street"] = "{0} {1} {2}".format(street_number, street_name, street_type)
    else:
...
def create_unstructured_params_list(keywords=None, suburb_name=None,
    ...
    if keywords:
        for keyword in keywords:
            params_dict = {}
            params_dict["q"] = "{0} {1} {2} {3}".format(keyword, suburb_name, state, country)
